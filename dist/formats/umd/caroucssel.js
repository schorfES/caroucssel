(function (global, factory) {
  ...
  class Mouse {
    ...
    _onDrag(event) {
      ...
      const {
        el
      } = fromCache(this, CACHE_KEY_PROXY$3);
  ...
  class Pagination {
    ...
    _add() {
      ...
      const pagination = render(template, {
        label,
        title,
        pages,
        className,
  ...
  class Mask {
    ...
    _render() {
      const {
        ...
        className,
        tagName
      } = fromCache(this, CACHE_KEY_CONFIGURATION$1);
      if (!enabled) {
        return;
      }
    ...
    _remove() {
      ...
      const {
        el
      } = fromCache(this, CACHE_KEY_PROXY$1);
  ...
  class Carousel {
    ...
    get items() {
      return fromCache(this, CACHE_KEY_ITEMS, () => {
        const {
          filterItem
        } = fromCache(this, CACHE_KEY_CONFIGURATION);
        const {
          el
        } = this;
      ...
      return fromCache(this, CACHE_KEY_PAGE_INDEX, () => {
        ...
        let visibles = index.reduce((acc, at) => {
          ...
          let {
            left,
            right
          } = items[at].getBoundingClientRect();
